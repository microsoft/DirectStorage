...
void LoadMaterials(MiniModel& model,
...
std::shared_ptr<Model> Renderer::LoadModel(const std::wstring& filePath, bool forceRebuild)
{
    ...
    std::shared_ptr<MiniModel> model(new MiniModel);
...
void MiniModel::FinalizeLoad()
{
    Model::m_MaterialConstants = m_MaterialConstants.GetGpuVirtualAddress();
    Model::m_DataBuffer = m_DataBuffer.GetGpuVirtualAddress();
    Model::m_MeshData = m_MeshData.get();
    Model::m_SceneGraph = m_SceneGraph.get();
    Model::m_KeyFrameData = m_KeyFrameData.get();
    Model::m_CurveData = m_CurveData.get();
    Model::m_Animations = m_Animations.get();
    Model::m_JointIndices = m_JointIndices.get();
    Model::m_JointIBMs = m_JointIBMs.get();
}
