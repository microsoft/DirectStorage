...
public:
    ...
    D3D12_GPU_VIRTUAL_ADDRESS m_MaterialConstants;
    D3D12_GPU_VIRTUAL_ADDRESS m_DataBuffer;
    uint8_t* m_MeshData = nullptr;
    GraphNode* m_SceneGraph = nullptr;
    uint8_t* m_KeyFrameData = nullptr;
    AnimationCurve* m_CurveData = nullptr;
    AnimationSet* m_Animations = nullptr;
    uint16_t* m_JointIndices = nullptr;
    Math::Matrix4* m_JointIBMs = nullptr;
};

// subclass of Model that owns all its own data
class MiniModel : public Model
{
public:
    virtual ~MiniModel() override = default;
