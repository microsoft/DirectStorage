...
public:
    ...
    void CreateFromResource(ID3D12Resource* resource, eDefaultTexture fallback, bool sRGB);
...
void ManagedTexture::CreateFromResource(ID3D12Resource* resource, eDefaultTexture fallback, bool sRGB)
{
    if (!resource)
    {
        m_hCpuDescriptorHandle = GetDefaultTexture(fallback);
    ...
    else
    {
        ...
        if (sRGB)
        {
            switch (desc.Format)
            {
                ...
                default:
                    ASSERT(false, "Expected an sRGB format");
                    break;
            }
        }
...
TextureRef TextureManager::LoadFromResource( char const* narrowPath, ID3D12Resource* resource, eDefaultTexture fallback, bool forceSRGB )
{
    ManagedTexture* tex = nullptr;
    {
        std::lock_guard<std::mutex> Guard(s_Mutex);

        std::wstring key = Utility::UTF8ToWideString(narrowPath);
        if (forceSRGB)
            key += L"_sRGB";
        ...
        if (it != s_TextureCache.end())
        {
            ...
            tex = it->second.get();
            ASSERT(resource == tex->GetResource());
            return tex;
        }
        else
        {
    ...
    tex->CreateFromResource(resource, fallback, forceSRGB);
    return tex;
}
