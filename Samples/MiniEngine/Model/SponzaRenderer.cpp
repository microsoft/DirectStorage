...
namespace Sponza
{
    ...
    ComPtr<ID3D12Fence> m_LoadTimerFence;
    CpuTimer m_LoadTimeToFirstFrame;
    HANDLE m_LoadTimerEvent;
    TP_WAIT* m_LoadTimerWait;
}

static DStorageLoadResult g_dstorageResult{};

static void OnLoadedFenceSignal(PTP_CALLBACK_INSTANCE, PVOID, PTP_WAIT, TP_WAIT_RESULT)
{
    Sponza::m_LoadTimeToFirstFrame.Stop();
    printf("Load time to first frame: %.2fs\n", Sponza::m_LoadTimeToFirstFrame.GetTime());
}

void Sponza::Startup( Camera& Camera, std::wstring const& modelFilename )
{    
    bool useDStorage = Utility::ToLower(Utility::GetFileExtension(modelFilename)) == L"h3da";
    if (useDStorage)
        InitializeDStorage();
    ...
    ASSERT_SUCCEEDED(
        Graphics::g_Device->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&m_LoadTimerFence)));        

    m_LoadTimeToFirstFrame.Start();

    m_LoadTimerEvent = ::CreateEvent(nullptr, FALSE, FALSE, nullptr);
    m_LoadTimerFence->SetEventOnCompletion(1, m_LoadTimerEvent);

    m_LoadTimerWait = CreateThreadpoolWait(OnLoadedFenceSignal, nullptr, nullptr);
    SetThreadpoolWait(m_LoadTimerWait, m_LoadTimerEvent, nullptr);

    if (useDStorage)
    {
        g_dstorageResult = DStorageLoadH3DAInto(&m_Model, modelFilename);
        ASSERT(g_dstorageResult.Succeeded, "Failed to load model");
    }
    else
    {
        ASSERT(m_Model.Load(modelFilename), "Failed to load model");
    }
...
{
    CloseThreadpoolWait(m_LoadTimerWait);
    CloseHandle(m_LoadTimerEvent);
    m_LoadTimerFence.Reset();
...
void Sponza::RenderScene(
    ...
    bool skipShadowMap)
{
    if (g_dstorageResult.Fence)
    {
        // Make sure that miniengine's graphic queue waits for the load to be
        // complete before continuing.  We don't need to block the CPU. We have
        // to do this as late as possible in the loading process since various
        // other steps will block waiting for work on these queues to complete.
        auto* fence = g_dstorageResult.Fence.Get();
        uint64_t value = g_dstorageResult.FenceValue;

        Graphics::g_CommandManager.GetGraphicsQueue().GetCommandQueue()->Wait(fence, value);

        // Clear the fence pointer so that we only do this once.
        g_dstorageResult.Fence.Reset();
    }

    Graphics::g_CommandManager.GetGraphicsQueue().GetCommandQueue()->Signal(m_LoadTimerFence.Get(), 1);
