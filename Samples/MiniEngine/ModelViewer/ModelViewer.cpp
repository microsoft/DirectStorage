...
void ModelViewer::Startup( void )
{
    ...
    if (CommandLineArgs::GetString(L"model", gltfFileName) == false)
    {
        ...
        // If no model file was specified default to using the compressed archived
        // sponza file which will use DirectStorage for loading.
        Utility::Printf(L"Loading using DirectStorage (zlib compressed archive)\n");
        Sponza::Startup(m_Camera, L"Sponza/sponza.zlib.h3da");
#else
...
#ifdef LEGACY_RENDERER
    else if (Utility::ToLower(Utility::GetFileExtension(gltfFileName)) == L"h3da")
    {
        Utility::Printf(L"Loading with DirectStorage\n");
        Sponza::Startup(m_Camera, gltfFileName);
        gltfFileName.clear();
    }
    else if (Utility::ToLower(Utility::GetFileExtension(gltfFileName)) == L"h3d")
    {
        Utility::Printf(L"Loading without using DirectStorage\n");
        Sponza::Startup(m_Camera, gltfFileName);
        gltfFileName.clear();
    }
#endif
    ...
    loadTimer.Stop();
    printf("Main thread loading Time: %2.2f seconds\n", loadTimer.GetTime());
