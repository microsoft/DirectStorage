...
struct TestConfig
{
    ...
    int gbvMaxMB = 4;                           // Max total *decompressed* size (MB) for Gbv tests. Larger files are skipped to avoid GBV TDRs (GBV slows the GPU, so hang risk tracks decompressed, not on-disk, size).
